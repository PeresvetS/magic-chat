...
class WABAOnlineStatusManager {
    ...
    async setOnline(userId, session) {
      try {
        ...
        logger.info(`Online status set for WABA user ${userId}, result: ${safeStringify(result)}`);

        return result;
      } catch (error) {
        logger.error(`Error setting online status for WABA user ${userId}:`, safeStringify(error));
        throw error;
      }
    }
    ...
    async setOffline(userId, session) {
      try {
        ...
        logger.error(`Error setting offline status for WABA user ${userId}:`, safeStringify(error));
      }
    }
