...
class OnlineStatusManager {
  ...
  async setOnline(userId, client) {
    try {
      ...
      this.onlineUsers.set(
        userId,
        setTimeout(() => this.setOffline(userId, client), this.timeoutDuration),
      );
      ...
      logger.error(
        `Error setting online status for WhatsApp user ${userId}:`,
        safeStringify(error),
      );
      throw error;
    }
  ...
  async setOffline(userId, client) {
    try {
      ...
      logger.error(
        `Error setting offline status for WhatsApp user ${userId}:`,
        safeStringify(error),
      );
    }
  }
