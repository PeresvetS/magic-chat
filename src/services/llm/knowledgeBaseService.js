...
class KnowledgeBaseService {
  constructor() {
    ...
    this.embeddings = new OpenAIEmbeddings({ openAIApiKey: config.OPENAI_API_KEY });
  }
  ...
  async createKnowledgeBase(name, description, campaignId, documents) {
    try {
      ...
      logger.info(`Created knowledge base: ${knowledgeBase.id} for campaign: ${campaignId}`);
  ...
  async getRelevantKnowledge(campaignId, query, maxBlocks) {
    try {
      const knowledgeBases = await knowledgeBaseRepository.findByCampaignId(campaignId);
      const pineconeIndex = this.pinecone.Index(config.PINECONE_INDEX);
      ...
      logger.info(`Retrieved ${limitedResults.length} relevant knowledge blocks for campaign: ${campaignId}`);
