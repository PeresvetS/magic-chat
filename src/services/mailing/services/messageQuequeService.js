...
async function processSingleQueueItem(queueItem) {
  ...
  if (result.success) {
    await RabbitMQQueueService.markAsCompleted(queueItem.id, {
      [queueItem.platform]: result,
    });
  } else if (result.error === 'DAILY_LIMIT_REACHED') {
    const newSenderPhoneNumber =
    ...
    if (newSenderPhoneNumber) {
      ...
      await RabbitMQQueueService.markAsFailed(
        queueItem.id,
