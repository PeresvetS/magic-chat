    ...
    try {
      ...
      const isRegistered = await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(
            () => reject(new Error('isRegisteredUser timeout')),
            30000,
          ),
        ),
      ]);
      ...
      logger.info(
        `Is number ${formattedNumber} registered on WhatsApp: ${isRegistered}`,
      );
      return isRegistered;
    } catch (error) {
      ...
      if (retries > 0) {
        logger.info(
          `Retrying WhatsApp check for ${phoneNumber}. Retries left: ${retries - 1}`,
        );
