...
module.exports = {
  '/create_leadsdb ([^\\s]+)': async (bot, msg, match) => {
    ...
    if (!leadsDBName) {
      bot.sendMessage(
        msg.chat.id,
        'Пожалуйста, укажите название для базы лидов. Например: /create_leadsdb МояБазаЛидов',
      );
      return;
    }
    ...
    try {
      ...
      bot.sendMessage(
        msg.chat.id,
        'Произошла ошибка при создании базы лидов. Пожалуйста, попробуйте позже.',
      );
    }
  },
  ...
  '/list_leadsdb': async (bot, msg) => {
    try {
      ...
      const leadsDBList = leadsDBs
        .map((db) => `- ${db.name} (ID: ${db.id})`)
        .join('\n');
      ...
      bot.sendMessage(
        msg.chat.id,
        'Произошла ошибка при получении списка баз лидов.',
      );
    }
  },
  ...
  '/upload_leads_to_db (.+)': async (bot, msg, match) => {
    ...
    try {
      ...
      bot.sendMessage(
        msg.chat.id,
        `Пожалуйста, отправьте Excel файл (XLS или XLSX) с лидами для базы лидов "${leadsDBName}"`,
      );
    } catch (error) {
  ...
  '/attach_leadsdb_to_campaign (.+) (.+)': async (bot, msg, match) => {
    ...
    try {
      ...
      bot.sendMessage(
        msg.chat.id,
        `База лидов "${leadsDBName}" успешно прикреплена к кампании "${campaignName}".`,
      );
    } catch (error) {
  ...
  '/detach_leadsdb_from_campaign (.+) (.+)': async (bot, msg, match) => {
    ...
    try {
      ...
      bot.sendMessage(
        msg.chat.id,
        `База лидов "${leadsDBName}" успешно откреплена от кампании "${campaignName}".`,
      );
    } catch (error) {
      bot.sendMessage(msg.chat.id, `Ошибка: ${error.message}`);
    }
  ...
  '/update_lead_status (.+) ([0-9]+) (NEW|UNAVAILABLE|PROCESSED_NEGATIVE|PROCESSED_POSITIVE)':
    async (bot, msg, match) => {
      ...
      try {
        ...
        bot.sendMessage(
          msg.chat.id,
          `Статус лида ${leadId} в базе "${leadsDBName}" обновлен на ${getStatusName(newStatus)}.`,
        );
      } catch (error) {
        logger.error('Error updating lead status:', error);
        bot.sendMessage(
          msg.chat.id,
          `Ошибка при обновлении статуса лида: ${error.message}`,
        );
      }
    },
  ...
  '/delete_lead (.+) ([0-9]+)': async (bot, msg, match) => {
    ...
    try {
      ...
      bot.sendMessage(
        msg.chat.id,
        `Лид с ID ${leadId} успешно удален из базы "${leadsDBName}".`,
      );
    } catch (error) {
      logger.error('Error deleting lead:', error);
      bot.sendMessage(
        msg.chat.id,
        `Ошибка при удалении лида: ${error.message}`,
      );
    }
  },
  ...
  '/get_default_leadsdb': async (bot, msg) => {
    try {
      ...
      bot.sendMessage(
        msg.chat.id,
        `Текущая дефолтная база лидов: ${defaultLeadsDB.name} (ID: ${defaultLeadsDB.id})`,
      );
    } catch (error) {
      logger.error('Error getting default LeadsDB:', error);
      bot.sendMessage(
        msg.chat.id,
        'Произошла ошибка при получении информации о дефолтной базе лидов. Пожалуйста, попробуйте позже.',
      );
    }
  },
    ...
    if (userState && userState.action === 'upload_leads_to_db') {
      if (
        ...
        try {
          ...
          bot.sendMessage(
            msg.chat.id,
            `Лиды успешно загружены в базу лидов (ID: ${userState.leadsDBId}). Добавлено ${addedLeadsCount} новых лидов.`,
          );
        } catch (error) {
        ...
        bot.sendMessage(
          msg.chat.id,
          'Пожалуйста, отправьте Excel файл (XLS или XLSX).',
        );
      }
    }
  },
