...
module.exports = {
  '/setlimit ([\\w\\.]+) (parsing|phones|campaigns|contacts|leads) (\\d+)': async (bot, msg, match) => {
    ...
    try {
      ...
      bot.sendMessage(msg.chat.id, `Лимит ${limitType} для ${userIdentifier} установлен на ${limitValue}.`);
    } catch (error) {
      bot.sendMessage(msg.chat.id, `Ошибка при установке лимита: ${error.message}`);
    }
  },
  ...
  '/getlimits ([\\w\\.]+)': async (bot, msg, match) => {
    ...
    try {
      ...
      bot.sendMessage(msg.chat.id, `Лимиты для ${userIdentifier}:\n${JSON.stringify(limits, null, 2)}`);
    } catch (error) {
      bot.sendMessage(msg.chat.id, `Ошибка при получении лимитов: ${error.message}`);
    }
  }
};