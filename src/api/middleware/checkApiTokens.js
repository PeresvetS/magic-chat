...
const checkBitrixToken = async (req, res, next) => {
  ...
  logger.info('Extracted authentication data', {
    auth: safeStringify(authFields),
  });
  ...
  if (!authFields.application_token) {
    ...
    return res
      .status(401)
      .json({ error: 'Invalid Bitrix24 application token' });
  }

  const bitrixOutboundToken = authFields.application_token;
  logger.info('Attempting to find user with Bitrix token', {
    token: bitrixOutboundToken,
  });
